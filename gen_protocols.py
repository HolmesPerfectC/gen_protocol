...
def gen_report_header(car_type, protocol, output_dir, protocol_template_dir):
    ...
    report_header_file = output_dir + "%s.hpp" % protocol["name"]
    with open(report_header_file, 'w') as h_fp:
        fmt_val = {}
        fmt_val["report_header_file_name_upper"] = protocol["name"].upper()
...
def gen_report_cpp(car_type, protocol, output_dir, protocol_template_dir):
    ...
    report_cpp_file = output_dir + "%s.cpp" % protocol["name"]
...
def gen_control_header(car_type, protocol, output_dir, protocol_template_dir):
    ...
    control_header_file = output_dir + "%s.hpp" % protocol["name"]
  ...
  %s %s(const std::uint8_t* bytes, const int32_t length) const;"""
        for var in protocol["vars"]:
            ...
            #                                 var["name"].lower())
            if "Message_check_sum" in var["name"]:
                fmtpub = "\n  // config detail: %s\n"
                fmtpri = "\n  // config detail: %s\n  void set_p_%s(uint8_t* data);"
                public_func_declare = fmtpub % str(var)
...
def gen_control_message_check_sum(var):
    """
        doc string:
    """
    impl = "\n"
    if "Message_check_sum" in var["name"]:
        impl = impl + "  u_char byte_sum = 0;\n"
        impl = impl + "  for (size_t i = 0; i < 7; i++)\n"
        impl = impl + "  {\n"
        impl = impl + "    byte_sum += data[i];\n"
        impl = impl + "  }\n"
        impl = impl + "  uint8_t x = static_cast<uint8_t>(byte_sum);"
    return impl + "\n"
...
void %(classname)s::set_p_%(var_name)s(uint8_t* data,
    ...
    if "Message_check_sum" in var["name"]:
        impl = impl + gen_control_message_check_sum(var)
    else:
...
def gen_control_cpp(car_type, protocol, output_dir, protocol_template_dir):
    ...
    control_cpp_file = output_dir + "%s.cpp" % protocol["name"]
    with open(control_cpp_file, 'w') as fp:
        ...
        for var in protocol["vars"]:
            ...
            if "Message_check_sum" in var["name"]:
                set_private_var = "  set_p_%s(data);" % var["name"].lower()
            else:
                set_private_var = "  set_p_%s(data, %s_);" % (var["name"].lower(),
                                                            var["name"].lower())
            set_private_var_list.append(set_private_var)
            init_val = "0"
            if "Message_Counter" in var["name"]:
                set_counter_var = "  %s_++;\n  if (%s_ > 3)\n  {\n    %s_ = 0;\n  }" % (var["name"].lower(), 
                                                                           var["name"].lower(),
                                                                           var["name"].lower())
                set_private_var_list.append(set_counter_var)
            if var["type"] == "double":
                init_val = "0.0"
            elif var["type"] == "bool":
                init_val = "false"
...
%s %s::%s(const std::uint8_t* bytes, int32_t length) const {"""
        ...
        message_counter_str = "message_counter"
        couter_elements = [x for x in set_private_var_list if message_counter_str in x]
        set_private_var_list = [x for x in set_private_var_list if message_counter_str not in x]
        set_private_var_list.extend(couter_elements)
        message_check_sum_str = "message_check_sum"
        matching_elements = [x for x in set_private_var_list if message_check_sum_str in x]
        set_private_var_list = [x for x in set_private_var_list if message_check_sum_str not in x]
        set_private_var_list.extend(matching_elements)
